...
const convert = [60 * 60, 60, 1, 1e6, 1e3, 1] as const;

export const printHRTime: any = (source: any, opts: any) => {
  let verbose,
    precise,
    i,
    spot,
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (i !== 3 && i !== 0) {
      sourceAtStep = sourceAtStep % (convert[i - 1] as number); // trim off previous portions
    }
    if (i === 2) {
      sourceAtStep += source[1] / 1e9; // get partial seconds from other portion of the array
    }
    valAtStep = sourceAtStep / (convert[i] as number); // val at this unit
    if (valAtStep >= 1) {
      if (verbose) {
